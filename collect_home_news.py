...
# 홍보성 이벤트 블랙리스트 — 뉴스가 아니라 세미나·특강·설명회·교육과정 모집 등 광고성이면 제외.
PROMO_TERMS = [
    "세미나", "특강", "설명회", "박람회", "컨퍼런스", "콘퍼런스", "강연회", "강좌",
    "아카데미", "웨비나", "수강생", "무료강의", "무료 강의", "무료특강", "무료 특강",
    "공모전", "출판기념", "시상식", "참가자 모집", "참가 신청", "수강 신청",
    # 경·공매 교육과정 모집 광고(뉴스 가장) — '왕초보 심화 과정 2기' 유형
    "왕초보", "수강료", "개강", "커리큘럼", "수강생 모집", "기수 모집",
    "심화과정", "기초과정", "실전과정", "입문과정", "환급과정", "원데이클래스",
]
# 정규식 보강: '과정 2기'·'3기 수강/모집' 등(단독 '3기'는 3기 신도시 오탐 방지 위해 제외).
...
# 언론사 섹션/형식 태그 — 제목 핵심어 판정에서 '내용어'로 오인되지 않게 판정 직전 제거.
#   🔴 2026-09-02 사고: "…목동윤슬자이[르포][부동산360] - 헤럴드경제"가 태그 '[부동산360]'(헤럴드 부동산
#   섹션 브랜드명)의 '부동산' 한 글자로 통과 → 특정 아파트 분양 홍보 르포가 홈에 노출. 태그를 떼면 제목에
#   진짜 부동산 핵심어가 0개라 정상 차단됨. '[전세사기]' 같은 실제 주제어 태그는 안 건드리도록 형식/섹션 태그만.
_BRAND_TAG_RE = re.compile(
    r"\[[^\]]*(?:부동산\d+|랜드|르포|단독|종합|속보|현장|기획|인터뷰|일문일답"
    r"|영상|포토|카드뉴스|사설|칼럼|기고|오피니언|이슈|핫이슈|뉴스|360)[^\]]*\]"
)


def _clean_title_for_match(title):
    """핵심어 판정용 제목 정제 — 언론 섹션/형식 대괄호 태그·끝의 ' - 언론사명' 꼬리 제거.
    (구글 뉴스 RSS 제목은 '본문제목 - 출처' 형식이라 끝의 ' - XXX'는 거의 항상 언론사명)"""
    t = _BRAND_TAG_RE.sub("", title or "")
    t = re.sub(r"\s*[-–—]\s*[가-힣A-Za-z0-9.· ]{1,20}$", "", t)   # 끝의 ' - 언론사' 제거
    return t

# 분양·입주 홍보성(광고 성격) — 특정 단지 세일즈 포인트 자랑이면 제외. '규제 회피'를 장점으로 내세우거나
#   호텔식 서비스 등 편의시설을 자랑하는 분양 홍보 르포. (주인님 지정 2026-09-02 '홍보성만 제외'.
#   분양가/청약경쟁률/미분양 통계 같은 시장 동향 뉴스는 여기 안 걸리게 보수적으로 최소만.)
_AD_TERMS_RE = re.compile(
    r"규제[도를]?\s*피[한해]|규제\s*(?:비껴|우회|회피)"        # 대출·세금 규제 '피한' 걸 세일즈 포인트로
    r"|호텔식|호텔[급\s]*서비스|호텔\s*서비스|컨시어지|조식\s*서비스"  # 호텔급 편의시설 자랑
)
...
def is_relevant(title, desc):
    ...
    if _PROMO_RE.search(t):                    # 세미나·특강·설명회 등 홍보성 이벤트 제외(뉴스 아님)
        return False
    if _AD_TERMS_RE.search(t):                 # 분양 홍보성(규제회피·호텔서비스 자랑) 제외 — 광고성 르포 차단
        return False
    if _POLITICS_RE.search(t):                 # 정쟁·선거·수사·국정평가 등 정치색 제외(부동산 표현 걸쳐도)
        return False
    if _CORE_RE.search(_clean_title_for_match(title)):   # 제목(브랜드태그 제거)에 핵심어 → 부동산 주제 확실
        return True
