...
def _chat_search_properties(args: dict, exclude_keys=None) -> dict:
    ...
    if args.get("has_expbid"):
        ...
        out = _we if _we else out   # 예상낙찰가 산정분 우선. 단 지역에 하나도 없으면 0건 대신 전체 유지('단기매매' 등에 has_expbid 잘못 붙어 다 빠지던 버그 — 지역에 산정분 없어도 물건은 보여줌)
    if _order == "profit":
