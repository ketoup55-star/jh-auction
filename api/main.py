...
def _user_grade_rank(user: Optional[dict]) -> int:
    ...
    grade_until(유지기한)이 지났으면 만료 → 무료(0) 대우(등급명 기록은 유지)."""
    try:
        u = user or {}
        gu = u.get("grade_until")
        if gu:
            import datetime as _dt
            try:
                if _dt.datetime.fromisoformat(gu) < _dt.datetime.now():
                    return 0                       # 등급 유지기한 만료 → 무료 대우
            except Exception:
                pass
        g = user_store._grade_by_name(u.get("grade") or "")
...
def _apt_deposit_unknown_keys() -> set:
    """보증금미상 조건 아파트 item_key — items.deposit_unknown 컬럼(백필) 단일쿼리. 기존 item_tenants(11.7만행) 스캔(1.08초·클라우드 0건고장) 대체.
    무거운 계산은 _apt_deposit_unknown_compute(백필 전용, _col_sync_loop 주기실행)로 분리."""
    keys: set = set()
    off = 0
    while True:
        try:
            r = auction_db._get("items", {"select": "item_key", "deposit_unknown": "is.true",
                                          "limit": "1000", "offset": str(off)})
            rows = r.json() if r.status_code in (200, 206) else []
        except Exception:
            break
...
def _sort_cols_backfill() -> None:
    ...
    try:
        ...
        with psycopg.connect(dburl, prepare_threshold=None, connect_timeout=15, autocommit=True) as c:
            ...
            # ⚠️단 source='api'(지번조회 표제부)는 같은 지번의 '다른 신축 건물'을 읽어 실제 물건보다 최대 60년 최신으로
            #   나오는 오류가 R2 전수대조로 확인됨(예: 실제 1955 → API 2020). R2 문서(doc/api+doc) 기반이거나
            #   items가 비었을 때만 반영해, R2 전수교정값이 API 값에 다시 덮이지 않게 한다(역주행 차단).
...
def _filter_cols_backfill() -> None:
    """필터 컬럼 주기 백필/재동기 — 기존 계산함수 재사용. 로컬 워머 전용(CLOUD_READER 쓰기금지).
    ①안정 컬럼(fuel/brand/car_ok/invest_amount): 신규 물건(invest_amount NULL)만 백필.
    ②멤버십 컬럼(over85_ok/deposit_unknown/zone): est·임차인·detail 변화로 멤버십이 바뀌므로 매 회 full 재동기(clear→set)."""
    if os.environ.get("CLOUD_READER", "0") in ("1", "true", "True"):
        return
    dburl = os.environ.get("SUPABASE_DB_URL")
    if not dburl:
...
def _apt_over85_keys() -> set:
    """아파트 85초과+차익 조건 item_key — items.over85_ok 컬럼(백필) 단일쿼리. 기존 전체 아파트 스캔+차익 인라인(2.7초) 대체.
    무거운 계산은 _apt_over85_compute(백필 전용, _col_sync_loop 주기실행)로 분리 = 필터는 항상 빠름·재발 없음."""
    keys: set = set()
    off = 0
    while True:
        try:
            r = auction_db._get("items", {"select": "item_key", "over85_ok": "is.true",
                                          "limit": "1000", "offset": str(off)})
            rows = r.json() if r.status_code in (200, 206) else []
        except Exception:
            break
...
class GradeIn(BaseModel):
    user_id: int
    grade: str
    days: Optional[int] = None   # 등급 유지일수(0/None=무제한, 관리자 임의 일수도 가능)
...
def admin_set_grade(body: GradeIn, admin: dict = Depends(require_admin)) -> dict:
    import datetime as _dt
    grade_until = None                                   # None = 무제한
    if body.days and int(body.days) > 0:
        grade_until = (_dt.datetime.now() + _dt.timedelta(days=int(body.days))).isoformat(timespec="seconds")
    u = user_store.set_grade(body.user_id, body.grade, grade_until)
    if not u:
