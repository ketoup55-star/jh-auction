...
#  🔴법원 문건접수 줄('2026-08-11 채권자 대항력포기확약서 제출')은 근거로 쓰지 않는다(주인님 지시 2026-09-27).
#   문건이 '접수'된 것과 법원이 매각조건으로 '인정'한 것은 다르다 — 인정되면 매각물건명세서에 적힌다.
#   실측 J06|2026|20123|1: 명세서엔 "배당에서 전액 변제되지 아니하면 잔액을 매수인이 인수함"인데
#   문건접수 목록의 확약서 줄 때문에 정반대로 '낙찰자 미인수'라고 표시됐다. 전수 1,072건이 이 경우였다.
#   detail_text의 '주의사항 / 법원문건접수 요약' 구간은 명세서 비고란('- '로 시작)과 문건접수 줄(날짜로 시작)이
#   섞여 있어 구간을 통째로 자르면 명세서 비고를 잃는다 → 날짜로 시작하는 줄만 걷어낸다.
_DOC_RECEIPT_LINE = re.compile(r"^\s*\d{4}[-./]\d{1,2}[-./]\d{1,2}\s+.*$", re.M)


def drop_doc_receipt(text: str) -> str:
    """법원 문건접수 줄 제거 — 확약서/인수조건 판단은 매각물건명세서 근거로만 한다."""
