...
def _occupant_key(name: str) -> str:
    ...
    """
    name = name or ""
    # '입주자:○○' + '주택도시보증공사(임차인김선영)'·'(임차인:김래영)' 표기(2026-09-22 실측: 괄호 안 '임차인'이 붙어 5글자라
    #  아래 2~4글자 규칙에 안 걸려 원임차인과 따로 남았다 → 같은 보증금 두 번 인수)
    m = re.search(r"(?:입주자|임차인|임차권자)[:\s]*([가-힣][가-힣,·]{1,30})\)?\s*$", name)   # 외국인 긴 이름(오스니흐홀랑)·공동임차(배정수,홍유진) 포함 or re.search(r"입주자[:\s]*([가-힣]{2,4})", name)
    if m:
        return m.group(1)
